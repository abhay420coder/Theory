...
export class WebsocketService {
  ...
  handleIncomingData(data: string){
    ...
    if(inData.message.serRelStatus){
      this.snackbar.open(inData.message.serRelStatus==="SUCCESS"?"Service Deployment Successful!":"Service Deployment Failed!", "Close", {duration: 3000});
    } else {
      this.snackbar.open(inData.message.status==="SUCCESS"?"Deployment Successful!":"Deployment Failed!", "Close", {duration: 3000});
    }
  }
