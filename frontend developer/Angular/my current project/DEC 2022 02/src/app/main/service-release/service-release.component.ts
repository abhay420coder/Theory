...
export class ServiceReleaseComponent implements OnInit {
  ...
  fetchEmployeeInfo(eidList: string[]){
    if(eidList.length!==0){
      this.api.postData(globalUrls.employeeDetailById, {employeeIdList: eidList}).subscribe((data:any)=>{
        const newData = data&&data.payload&&data.payload.list?data.payload.list:[];
  ...
  getUserName(lastModifiedBy: string){
    if(!lastModifiedBy){
      return "Unknown";
    }
    const eid = lastModifiedBy.split(":")[0];
    const empDetail = this.employeeData.filter(d=>d&&d.employeeId&&d.employeeId.toString() === eid)[0];
    return empDetail&&empDetail.employeeName?empDetail.employeeName:"Unknown";
  }
